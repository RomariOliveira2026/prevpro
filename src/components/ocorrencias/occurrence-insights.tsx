...
import type { OccurrenceInsight } from "@/lib/ocorrencias-data";

function InsightIcon({ icon }: { icon: OccurrenceInsight["icon"] }) {
...
const iconStyles: Record<OccurrenceInsight["icon"], string> = {
  chart: "bg-prevpro-blue/10 text-prevpro-blue group-hover:bg-prevpro-blue group-hover:text-white",
  sector: "bg-amber-50 text-amber-600 group-hover:bg-amber-500 group-hover:text-white",
  savings: "bg-emerald-50 text-prevpro-green group-hover:bg-prevpro-green group-hover:text-white",
};

interface OccurrenceInsightsProps {
  insights: OccurrenceInsight[];
}

export function OccurrenceInsights({ insights }: OccurrenceInsightsProps) {
  return (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-slate-500">
              Análises geradas a partir dos registros
            </p>
          </div>
        </div>
      </div>
